...
$(document).ready(function() {
	//高亮左侧菜单
	App.activeMenu("sys/user/slist");
	Page.initData(
		{
			url:"${ctx}/sys/user/spage",
			tableId : "#sample_1",
			pageSize : 10
		},
		null,
		[
		 ...
		 {cName:"status",cValue:"操作(是否是会员)",format:function(i,value,item){
			 ...
			 if(value === 2){
				 return $a.clone().attr("data-original-title","点击成为VIP").addClass("grey").html('<i class="icon-unlock"></i>点击成为VIP(非VIP)');
			 }
	...
	var callback = function(result){
		...
		$.ajax({
			...
			success : function(data){
				if(data == 3){
					$(obj).removeAttr("data-original-title").attr("data-original-title","点击取消VIP资格").removeClass("grey").addClass("green").html('<i class="icon-unlock"></i>点击取消VIP资格(是VIP)');
				}else{
					$(obj).removeAttr("data-original-title").attr("data-original-title","点击成为VIP").removeClass("green").addClass("grey").html('<i class="icon-lock"></i>点击成为VIP（非VIP）');
				}
				unBlockUI();
			}
		});

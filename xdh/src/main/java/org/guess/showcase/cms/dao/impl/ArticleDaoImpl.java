...
public class ArticleDaoImpl extends HibernateDao<Article, Long> implements
	...
	@Override
	public Article getLastArtcileByCategoryId(Long categoryId) {
		String sql = "from Article as a where a.category.id = " + categoryId + " order by id desc limit 1";
		
		List<Article> list = getSession()
				.createQuery(sql)
				.list();
		return list.get(0);
	}

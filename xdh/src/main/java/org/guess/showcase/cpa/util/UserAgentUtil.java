package org.guess.showcase.cpa.util;
...
public class UserAgentUtil {  
    ...
    public static UserAgent getUserAgent(String userAgent) {  
        ...
        if(checkAgent(userAgent)){
        	return parseMobile(userAgent);
        }else{
        	return parsePc(userAgent);
        }
    }  
    
    private static UserAgent parseMobile(String userAgent){
    	// IOS 判断字符串  
        String iosString = "LIKE MAC OS X";  
        if (userAgent.contains(iosString)) { 
        	String platformType = "";
        	...
        	return judgeBrowser(userAgent, platformType, null , null);
        	
              
        }  
        // Android 判断  
        String androidString = "ANDROID";  
        if (userAgent.contains(androidString)) {  
        	return judgeBrowser(userAgent, androidString, null , null); 
        }  
        // windows phone 判断  
        String wpString = "WINDOWS PHONE";  
        if (userAgent.contains(wpString)) {  
        	return judgeBrowser(userAgent, wpString, null , null); 
        }  
        // BlackBerry 黑莓系统判断  
        String bbString = "BLACKBERRY";  
        if (userAgent.contains(bbString)) {  
        	return judgeBrowser(userAgent, bbString, null , null); 
        }  
        
        if(userAgent.contains("LINUX")){//android  
        	return judgeBrowser(userAgent, "ANDROID", null , null); 
        }  
          
        //UCWEB/2.0 (iOS; U; iPh OS 4_3_2; zh-CN; iPh4)  
        if(userAgent.matches(".*((IOS)|(IPAD)).*(IPH).*")){  
        	return judgeBrowser(userAgent, "IOS", null , null);
        } 
    	
    	return null;
    }
    
    private static UserAgent parsePc(String userAgent){
    ...
    public static boolean checkAgent(String userAgent){   
    	boolean flag =false;
    	String[] items = new String[]{"ANDROID", "IPAD", "IPHONE","IOS","BLACKBERRY","WINDOWS PHONE", "IPH" };
    ...
    public static void main(String[] args) {
		String ua = "Mozilla/5.0 (Linux; Android 5.0.2; CHE-TL00H Build/HonorCHE-TL00H) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/37.0.0.0 Mobile Safari/537.36";
		ua = ua.toUpperCase();
		System.out.println(new Gson().toJson(getUserAgent(ua)));
	}
}
